...
\begin{definition}{6.22}[orthonormal] \enumfix
\begin{enumerate}
\item A list of vectors is called \defn{orthonormal} if each vector in the list has norm 1 and is orthogonal to all the other vectors in the list.
...
\begin{definition}{6.27}[orthonormal basis]
An \defn{orthonormal basis} of $V$ is an orthonormal list of vectors in $V$ that is also a basis of $V$.
\end{definition}

\begin{definition}{6.39}[linear functional, dual space, $V'$] \enumfix
\begin{enumerate}
\item A \defn{linear functional} on $V$ is a linear map from $V$ to $F$.
\item The \defn{dual space} of $V$, denoted by $V'$, is the vector space of all linear functionals on $V$. In other words, $V' = \L(V, \F)$.
\end{enumerate}
\end{definition}

\vspace{20\baselineskip}

\begin{result}{6.24}[norm of an orthonormal linear combination]
Suppose $e_1, \ldots, e_m$ is an orthonormal list of vectors $V$. Then
$$
...
\begin{corollary}{6.25}[orthonormal lists are linearly independent]
Every orthonormal list of vectors is linearly independent.
\end{corollary}

% Exercise 1
\begin{result}{Ex. 6B, 1}[equivalence of square norm sums implies orthonormality]
Suppose $e_1, \ldots, e_m$ is a list of vectors in $V$ such that
$$
\norm{a_1 e_1 + \cdots + a_m e_m}^2 = | a_1 |^2 + \cdots | a_m |^2
$$
for all $a_1, \ldots, a_m \in \F$. Then $e_1, \ldots, e_m$ is an orthonormal list.
\end{result}

\newpage

\begin{result}{6.26}[Bessel's inequality]
Suppose $e_1 \ldots, e_m$ is an orthonormal list of vectors in $V$. If $v \in V$ then
$$
| \inner{v}{e_1} |^2 + \cdots + | \inner{v}{e_m} |^2 \le \norm{v}^2 .
$$
\end{result}

\begin{result}{6.28}[orthonormal lists of the right length are orthonormal bases]
Suppose $V$ is finite-dimensional. Then every orthonormal list of vectors in $V$ of length $\dim V$ is an orthonormal basis of $V$.
\end{result}

\begin{result}{6.30}[writing a vector as a linear combination of an orthonormal basis]
Suppose $e_1, \ldots, e_n$ is in orthonormal basis $V$ and $u,v \in V$. Then
\begin{enumerate}
\item[(a)] $v = \inner{v}{e_1} e_1 + \cdots + \inner{v}{e_n} e_n,$
\item[(b)] $\norm{v}^2 = | \inner{v}{e_1} |^2 + \cdots + | \inner{v}{e_n} |^2,$
\item[(c)] $\inner{u}{v} = \inner{u}{e_1} \overline{\inner{v}{e_1}} + \cdots + \inner{u}{e_n} \overline{\inner{v}{e_n}}.$
\end{enumerate}
\end{result}

\begin{result}{6.32}[Gram-Schmidt procedure]
...
$$
f_k = v_k - \frac{\inner{v_k}{f_1}}{\norm{f_1}^2} f_1 - \cdots - \frac{\inner{v_k}{f_{k - 1}}}{\norm{f_{k - 1}}^2} f_{k - 1} .
$$
For each $k = 1, \ldots, m$, let $e_k = \frac{f_k}{\norm{f_k}}$. Then $e_1, \ldots, e_m$ is an orthonormal list of vectors $V$ such that
$$
\vspan(v_1, \ldots, v_k) = \vspan(e_1, \ldots, e_k)
$$
for each $k = 1, \ldots, m$.
\end{result}

\begin{result}{6.35}[existence of orthonormal basis]
Every finite-dimensional inner product space has an orthonormal basis.
\end{result}

\begin{result}{6.36}[every orthonormal list extends to an orthonormal basis]
Suppose $V$ is finite-dimensional. Then every orthonormal list of vectors in $V$ can be extended to an orthonormal basis of $V$.
\end{result}

\begin{result}{6.37}[upper-triangular matrix with respect to some orthonormal basis]
Suppose $V$ is finite-dimentional and $T \in \L(V)$. Then $T$ has an upper-triangular matrix with respect to some orthonormal basis of $V$ if and only if the minimal polynomial of $T$ equals $(z - \lambda_1) \cdots (z - \lambda_m)$ for some $\lambda_1, \ldots, \lambda_m \in \F$.
\end{result}

\begin{theorem}{6.38}[Schur's theorem]
...
\begin{theorem}{6.42}[Riesz representation theorem]
...
\begin{definition}{6.46}[orthogonal complement, $U^\bot$]
If $U$ is a subset of $V$, then the \defn{orthogonal complement\/} of $U$, denoted by $U^\bot$, is the set of all vectors in $V$ that are orthogonal to every vector in $U$:
$$
U^\bot = \{ v \in V \setsep \inner{u}{v} = 0 \text{ for every } u \in U \}.
$$
\end{definition}

\begin{definition}{6.55}[orthogonal projection, $P_U$]
Suppose $U$ is a finite-dimensional subspace of $V$. The \defn{orthogonal projection\/} of $V$ onto $U$ is the operator $P_U \in \L(V)$ defined as follows: For each $v \in V$, write $v = u + w$, where $u \in U$ and $w \in U^\bot$. Then $P_U v = u$.
\end{definition}

\begin{definition}{6.68}[pseudoinverse, $T^\dagger$]
Suppose that $V$ is finite-dimensional and $T \in \L(V, W)$. The \defn{pseudoinverse} $T^\dagger \in \L(W, V)$ of $T$ is the linear map from $W$ to $V$ defined by
$$
T^\dagger w = (T|_{(\kernel T)^\bot)})^{-1} P_{\range T} w
$$
for each $w \in W$.
\end{definition}
...
\begin{result}{6.48}[properties of orthogonal complement] \enumfix
\begin{enumerate}
\item[(a)] If $U$ is a subset of $V$, then $U^\bot$ is a subspace of $V$.
\item[(b)] $\{ 0 \}^\bot = V$.
\item[(c)] $V^\bot = \{ 0 \}$.
\item[(d)] If $U$ is a subset of $V$, then $U \cap U^\bot \subseteq \{ 0 \}$.
\item[(e)] If $G$ and $H$ are subsets of $V$ and $G \subseteq H$, then $H^\bot \subseteq G^\bot$.
\end{enumerate}
\end{result}

\begin{result}{6.49}[direct sum of a subspace and its orthogonal complement]
...
\begin{result}{6.51}[dimension of orthogonal complement]
Suppose $V$ is finite-dimensional and $U$ is a subspace of $V$. Then
$$
...
\begin{result}{6.52}[orthogonal complement of the orthogonal complement]
Suppose $U$ is a fintite-dimensional subspace of $V$. Then
$$
U = \left( U^\bot \right)^\bot.
$$
\end{result}

\begin{result}{6.54}[$U^\bot = \{ 0 \} \Longleftrightarrow U = V$ (for $U$ a finite-dimensional subspace of $V$)]
Suppose $U$ is a finite-dimensional subspace of $V$. Then
$$
U^\bot = \{ 0 \} \Longleftrightarrow U = V.
$$
\end{result}

\begin{result}{6.57}[properties of orthogonal projection $P_U$] \enumfix
\begin{enumerate}
\item[(a)] $P_U \in \L(V)$;
\item[(b)] $P_U u = u$ for every $u \in U$;
\item[(c)] $P_U w = 0$ for every $w \in U^\bot$;
\item[(d)] $\range P_U = U$;
\item[(e)] $\kernel P_U = U^\bot$;
\item[(f)] $v - P_U v \in U^\bot$ for every $v \in V$;
\item[(g)] $P_U^2 = P_U$;
\item[(h)] $\norm{P_U v} \le \norm{v}$ for every $v \in V$;
\item[(i)] if $e_1, \ldots, e_m$ is an orthonormal basis of $U$ and $v \in V$, then
$$
P_U v = \inner{v}{e_1} e_1 + \cdots + \inner{v}{e_m} e_m.
$$
\end{enumerate}
\end{result}

\begin{theorem}{6.58}[Riesz representation theorem, revisited]
Suppose $V$ is finite-dimensional. For each $v \in V$, define $\varphi_v \in V'$ by
$$
\varphi_v (u) = \inner{u}{v}
$$
for each $u \in V$. Then $v \to \varphi_v$ is a one-to-one function from $V$ to $V'$.
\end{theorem}

\begin{result}{6.61}[minimising distance to a subspace]
...
\begin{result}{6.67}[restriction of a linear map to obtain a one-to-one and onto map]
Suppose $V$ is finite-dimensional and $T \in \L(V, W)$. Then $T|_{(\kernel T)^\bot}$ is a one-to-one map of $(\kernel T)^\bot$ onto $\range T$.
\end{result}

\begin{result}{6.69}[algebraic properties of the pseudoinverse]
Suppose $V$ is finite-dimensional and $T \in \L(V, W)$.
\begin{enumerate}
\item[(a)] If $T$ is invertible, then $T^\dagger = T^{-1}$.
\item[(b)] $T T^\dagger = P_{\range T} = $ the orthogonal projection of $W$ onto $\range T$.
\item[(c)] $T^\dagger T = P_{(\kernel T)^\bot} =$ the orthogonal projection of $V$ onto $(\kernel T)^\bot$.
\end{enumerate}
\end{result}

\begin{result}{6.70}[pseudoinverse provides best approximate solution or best solution]
Suppose $V$ is finite-dimensional, $T \in \L(V, W)$, and $b \in W$.
\begin{enumerate}
\item[(a)] If $x \in V$, then
$$
\norm{T (T^\dagger b) - b} \le \norm{T x - b},
$$
with equality if and only if $x \in T^\dagger b + \kernel T$.
\item[(b)] If $x \in T^\dagger b + \kernel T$, then
$$
\norm{T^\dagger b} \le \norm{x},
$$
...
\begin{result}{7.26}[invertible quadratic expressions]
Suppose $T \in \L(V)$ is self-adjoint and $b, c \in \R$ are such that $b^2 < 4 c$. Then
$$
T^2 + b T + c I
$$
...
\begin{result}{7.27}[minimal polynomial of self-adjoint operator]
Suppose $T \in \L(V)$ is self-adjoint. Then the minimal polynomial of $T$ equals $(z - \lambda_1) \cdots (z - \lambda_m)$ for some $\lambda_1, \ldots, \lambda_m \in \R$.
\end{result}

\begin{theorem}{7.29}[real spectral theorem]
...
\begin{theorem}{7.31}[complex spectral theorem]
...
\begin{definition}{7.34}[positive operator]
An operator $T \in \L(V)$ is called \defn{positive} if $T$ is self-adjoint and
$$
\inner{T v}{v} \ge 0
$$
for all $v \in V$.
...
\begin{definition}{7.36}[square root]
An operator $R$ is called a \defn{square root} of an operator $T$ if $R^2 = T$.
\end{definition}

\begin{notation}{7.40}[$\sqrt{T}$]
For $T$ a positive operator, $\sqrt{T}$ denotes the unique positive square root of $T$.
\end{notation}

\vspace{10\baselineskip}

\begin{result}{7.38}[characterisation of positive operators]
...
\item[(f)] $T = R^* R$ for some $R \in \L(V)$.
\end{enumerate}
\end{result}

\begin{result}{7.39}[each positive operator has only one positive square root]
Every positive operator on $V$ has a unique positive square root.
\end{result}

\begin{result}{7.43}[$T$ positive and $\inner{T v}{v} = 0 \Longrightarrow T v = 0$]
Suppose $T$ is a positive operator on $V$ and $v \in V$ is such that $\inner{T v}{v} = 0$. Then $T v = 0$.
\end{result}
...
\begin{definition}{7.44}[isometry]
A linear map $S \in \L(V, W)$ is called an \defn{isometry} if
$$
\norm{S v} = \norm{v}
$$
...
\begin{definition}{7.51}[unitary operator]
An operator $S \in \L(V)$ is called \defn{unitary} if $S$ is an invertible isometry.
\end{definition}

\begin{definition}{7.56}[unitary matrix]
An $n$-by-$n$ matrix is called \defn{unitary} if its columns form an orthonormal list in $\F^n$.
\end{definition}

\begin{definition}{7.62}[positive definite]
A matrix $B \in \F^{n, n}$ is called \defn{positive definite} if $B^* = B$ and
$$
\inner{B x}{x} > 0
$$
...
\begin{result}{7.49}[characterisation of isometries]
Suppose $S \in \L(V, W)$. Suppose $e_1, \ldots, e_n$ is an orthonormal basis of $V$ and $f_1, \ldots, f_m$ is an orthnormal basis of $W$. Then the following are equivalent.
\begin{enumerate}
\item[(a)] $S$ is an isometry.
\item[(b)] $S^* S = I$.
\item[(c)] $\inner{S u}{S v} = \inner{u}{v}$ for all $u, v \in V$.
\item[(d)] $S e_1, \ldots, S e_n$ is an orthonromal list in $W$.
\item[(e)] The columns of $\M(S, (e_1, \ldots, e_n), (f_1, \ldots, f_m))$ form an orthonormal list in $\F^m$ with respect to the Euclidean inner product.
\end{enumerate}
\end{result}

\begin{result}{7.53}[characterisation of unitary operators]
Suppose $S \in \L(V)$. Suppose $e_1, \ldots, e_n$ is an orthnormal basis of $V$. Then the following are equivalent.
\begin{enumerate}
\item[(a)] $S$ is a unitary operator.
\item[(b)] $S* S = S S* = I$.
\item[(c)] $S$ is invertible and $S^{-1} = S^*$.
\item[(d)] $S e_1, \ldots, S e_n$ is an orthonormal basis of $V$.
\item[(e)] The rows of $\M(S, (e_1, \ldots, e_n))$ form an orthonormal basis of $\F^n$ with respect to the Euclidean inner product.
\item[(f)] $S^*$ is a unitary operator.
\end{enumerate}
\end{result}

\begin{result}{7.54}[eigenvalues of unitary operators have absolute value 1]
Suppose $\lambda$ is an eigenvalue of a unitary operator. Then $| \lambda | = 1$.
\end{result}

\begin{result}{7.55}[description of unitary operators on complex inner product spaces]
Suppose $\F = \C$ and $S \in \L(V)$. Then the following are equivalent.
\begin{enumerate}
\item[(a)] $S$ is a unitary operator.
\item[(b)] There is an orthonormal basis of $V$ consisting of eigenvectors of $S$ whose corresponding eigenvalues all have asbolute value 1.
\end{enumerate}
\end{result}

\begin{result}{7.57}[characterisation of unitary matrices]
Suppose $Q$ is an $n$-bu-$n$ matrix. Then the following are equivalent.
\begin{enumerate}
\item[(a)] $Q$ is a unitary matrix.
\item[(b)] The rows of $Q$ form an orthonormal list in $\F^n$.
\item[(c)] $\norm{Q v} = \norm{v}$ for every $v \in \F^n$
\item[(d)] $Q^* Q = Q Q^* = I$, the $n$-by-$n$ matrix with 1's on the diagonal and 0's elsewhre.
\end{enumerate}
\end{result}

\begin{result}{7.58}[QR factorisation]
Suppose $A$ is a square matrix with linearly independent columns. Then there exis unique matrices $Q$ and $R$ such that $Q$ is unitary, $R$ is upper triangular with only positive numbers on its diagonal, and
$$
A = Q R.
$$
\end{result}

\begin{result}{7.61}[positive invertible operator]
A self-adjoint operator $T \in \L(VB)$ is a positive invertible operator if and only if $\inner{T v}{v} > 0$ for every nonzero $v \in V$.
\end{result}

\begin{result}{7.63}[Cholesky factorisation]
...
\begin{definition}{7.65}[singular values]
Suppose $T \in \L(V, W)$. The \defn{singular values} of $T$ are the nonegative square roots of the eigenvalues of $T^* T$, listed in decreasing order, each included as many times as the dimension of the corresponding eigenspace of $T^* T$.
\end{definition}

\begin{definition}{7.74}[diagonal matrix]
An $M$-by-$N$ matrix $A$ is called a \defn{diagonal mtarix} if all entries of the matrix are 0 except possibly $A_{k, k}$ for $k = 1, \ldots, \min \{M, N \}$.
\end{definition}

\newpage

\begin{result}{7.64}[properties of $T^* T$]
Suppose $T \in \L(V, W)$. Then
\begin{enumerate}
\item[(a)] $T^* T$ is a positive operator on $V$;
\item[(b)] $\kernel T^* T = \kernel T$;
\item[(c)] $\range T^* T = \range T^*$;
\item[(d)] $\dim \range T = \dim \range T^* = \dim \range T^* T$.
\end{enumerate}
\end{result}

\begin{result}{7.68}[role of positive singular valuse]
Suppose $T \in \L(V, W)$. Then
\begin{enumerate}
\item[(a)] $T$ is injective $\Longleftrightarrow$ 0 is not a singular value of $T$;
\item[(b)] the number of positive singular values of $T$ equals $\dim \range T$;
\item[(c)] $T$ is surjective $\Longleftrightarrow$ number of positive singular values of $T$ equals $\dim W$.
\end{enumerate}
\end{result}

\begin{result}{7.69}[isometries characterised by having all singular values equal to 1]
Suppose that $S \in \L(V, W)$. Then
$$
\text{$S$ is an isometry} \Longleftrightarrow \text{all singular values of $S$ equal 1}.
$$
\end{result}

\begin{result}{7.70}[singular value decomposition]
Suppose $T \in \L(V, W)$ and the positive singular values of $T$ are $s_1, \ldots, s_m$. Then there exist orthonormal lists $e_1, \ldots, e_m$ in $V$ and $f_1, \ldots. f_m$ in $W$ such that
\begin{equation} \tag{7.71}
T v = s_1 \inner{v}{e_1} f_1 + \cdot + s_m \inner{v}{e_m} f_m
\end{equation}
for every $v \in V$.
\end{result}

\begin{result}{7.75}[singular value decomposition of adjoint and pseudoinverse]
Suppose $T \in \L(V, W)$ and the positive singular values of $T$ are $s_1, \ldots, s_m$. Suppose $e_1, \ldots, e_m$ and $f_1, \ldots, f_m$ are orthonormal lists in $V$ and $W$ such that
\begin{equation} \tag{7.76}
T v = s_1 \inner{v}{e_1} f_1 + \cdots + s_m \inner{v}{e_m} f_m
\end{equation}
for every $v \in V$. Then
\begin{equation} \tag{7.77}
T^* w = s_1 \inner{w}{f_1} e_1 + \cdots + s_m \inner{w}{f_m} e_m
\end{equation}
and
\begin{equation} \tag{7.78}
T^\dagger w = \frac{\inner{w}{f_1}}{s_1} e_1 + \cdots \frac{\inner{w}{f_m}}{s_m} e_m
\end{equation}
for every $w \in W$.
\end{result}

\begin{result}{7.80}[matrix versoin of SVG]
...
\begin{definition}{7.86}[norm of a linear map, $\norm{\cdot}$]
Suppose $T \in \L(V, w)$. Then the \defn{norm} of $T$, deonted by $\norm{T}$, is defined by
$$
\norm{T} = \max \{ \norm{T v} \setsep v \in V \text{ and } \norm{v} \le 1 \}.
$$
\end{definition}

\newpage

\begin{result}{7.82}[upper bound for $\norm{T v}$]
Suppose $T \in \L(V, W)$. Let $s_1$ be the largest singular value of $T$. Then
$$
\norm{T v} \le s_v \norm{v}
$$
for all $v \in V$.
...
\begin{result}{7.87}[basic properties of norms of linear maps]
Suppose $T \in \L(V, W)$. Then
\begin{enumerate}
\item[(a)] $\norm{T} \ge 0$;
\item[(b)] $\norm{t} = 0 \Longleftrightarrow T = 0$;
\item[(c)] $\norm{\lambda T} = | \lambda | \norm{T}$ for all $\lambda \in \F$;
\item[(d)] $\norm{S + T} \le \norm{T} + \norm{T}$ for all $S \in \L(V, W)$.
\end{enumerate}
\end{result}

\begin{result}{7.91}[norm of the adjoint]
Suppose $T \in \L(V, W)$. Then $\norm{T^*} = \norm{T}$.
\end{result}

\begin{result}{7.92}[best approximation by linear map whose range has dimension $\le k$]
Suppose $T \in \L(V, W)$ and $s_1 \ge \cdots \ge s_m$ are the positive singular values of $T$. Suppose $1 \le k < m$. Then
$$
\min \{ \norm{T - S} \setsep S \in \L(V, W) \text{ and } \dim \range S \le k \} = s_{k + 1} .
$$
Furthermore, if
$$
T v = s_1 \inner{v}{e_1} f_1 + \cdots + s_k \inner{v}{e_k} f_k
$$
is a singular value decomposition of $T$ and $T_k \in \L(V, W)$ is defined by
$$
T_k v = s_1 \inner{v}{e_1} f_1 + \cdots + s_k \inner{v}{e_k} f_k
$$
for each $v \in V$, then $\dim \range T_k = k$ and $\norm{T - T_k} = s_{k + 1}$.
\end{result}

\begin{result}{7.93}[polar decomposition]
...
\begin{definition}{7.95}[ball, $B$]
The \defn{ball} in $V$ of radius 1 centered on 0, denoted by $B$, is defined by
$$
B = \{ v \in V \setsep \norm{v} < 1 \}.
$$
\end{definition}

\begin{definition}{7.96}[ellipsoid, $E(s_1 f_1, \ldots, s_n f_n)$, principal axes]
Suppose that $f_1, \ldots, f_m$ is an orthonormal basis of $V$ and $s_1, \ldots, s_n$ are positive numbers. The \defn{elliposoid} $E(s_1 f_1, \ldots, s_n f_n)$ with \defn{principal axes} $s_1, f_1, \ldots, s_n f_n$ is defined by
\begin{align*}
& E(s_1 f_1, \ldots, s_n f_n) = \\
& \qquad \left\{ v \in V \setsep \frac{|\inner{v}{f_1}|^2}{s_1^2} + \cdots + \frac{|\inner{v}{f_n}|^2}{s_n^2} < 1 \right\} .
\end{align*}
\end{definition}

\begin{notation}{7.98}[$T(\Omega)$]
For $T$ a function defined on $V$ and $\Omega \subseteq V$, define $T(\Omega)$ by
$$
T(\Omega) = \{ T v \setsep v \in \Omega \} .
$$
\end{notation}

\begin{definition}{7.102}[$P(v_1, \ldots, v_n)$, parallelepiped]
Suppose $v_1, \ldots, v_n$ is a basis for $V$. Let
$$
P(v_1, \ldots, v_n) = \{ a_1 v_1 + \cdots + a_n v_n \setsep a_1, \ldots, a_n \in (0, 1) \} .
$$
A \defn{parallelepiped} is a set of the form $u + P(v_1, \ldots, v_n)$ for some $u \in V$. The vectors $v_1, \ldots, v_n$ are called the \defn{edges} of this parallelepiped.
\end{definition}

\begin{definition}{7.105}[box]
A \defn{box} in $V$ is a set of the form
$$
u + P(r_1 e_1, \ldots, r_n e_n),
$$
where $u \in V$ and $r_1, \ldots, r_n$ are positive numbers and $e_1, \ldots, e_n$ is an orthonormal basis of $V$.
\end{definition}

\begin{definition}{7.108}[volume of abox]
Suppose $\F = \R$. If $u \in V$ and $r_1, \ldots, r_n$ are positive numbers and $e_1, \ldots, e_n$ is an orthonormal basis of $V$, then
$$
\volume(u + P(r_1 e_1, \ldots, r_n e_n)) = r_1 \times \cdots \times r_n .
$$
\end{definition}

\begin{definition}{7.109}[voolume]
Suppose $\F = \R$ and $\Omega \subseteq V$. Then the \defn{volume} of $\Omega$, denoted by $\volume \Omega$, is approximately the sum of the volumes of a collection of disjoint boxes that approximate $\Omega$.
\end{definition}

\newpage

\begin{result}{7.99}[invertible operator takes ball to ellipsoid]
Suppose $T \in \L(V)$ is invertible. Then $T$ maps the ball $B$ in $V$ onto an ellipsoid in $V$.
\end{result}

\begin{result}{7.101}[invertible operator takes ellipsoids to ellipsoids]
Suppose $T \in \L(V)$  is invertible and $E$ is an ellipsoid in $V$. Then $T(E)$ is an ellipsoid in $V$.
\end{result}

\begin{result}{7.104}[invertible operator takes parallelepipeds to parallelepipeds]
Suppose $u \in V$ and $v_1, \ldots, v_n$ is a basis of $V$. Suppose $T \in \L(V)$ is invertible. Then
$$
T(u + P(v_1, \ldots, v_n)) = T u + P( T v_1, \ldots, T v_n ) .
$$
\end{result}

\begin{result}{7.107}[every invertible operator takes some boxes to boxes]
Suppose $T \in L(V)$ is invertible. Suppose $T$ has singular value decomposition
$$
T v = s_1 \inner{v}{e_1} f_1 + \cdots + s_n \inner{v}{e_n} f_n ,
$$
where $s_1, \ldots, s_n$ are singular values of $T$ and $e_1, \ldots, e_n$ and $f_1, \ldots, f_n$ are orthonormal bases of $V$ and the equation above holds for all $v \in V$. Then $T$ maps the box $u + P(r_1 e_1, \ldots, r_n e_n)$ onto the box $T u + P(r_1 s_1 f_1, \ldots, r_n s_n f_n)$ for all positive numbers $r_1, \ldots, r_n$ and all $u \in V$.
\end{result}

\begin{result}{7.111}[volume changes by a factor of the product of the singular values]
Suppose $\F = \R, T \in \L(V)$ is invertible, and $\Omega \subseteq V$. Then
$$
\volume T(\Omega) = ( \text{product of singular values of $T$} (\volume \Omega) .
$$
\end{result}
